...
class purchase_order(models.Model):
    ...
    def default_sh_purchase_bm_is_cont_scan(self):
        if self.env.user and self.env.user.company_id:
            return self.env.user.company_id.sh_purchase_bm_is_cont_scan
    
    sh_purchase_barcode_mobile = fields.Char(string = "Mobile Barcode")
    
    sh_purchase_bm_is_cont_scan = fields.Char(string='Continuously Scan?',default = default_sh_purchase_bm_is_cont_scan, readonly=True)
    ...
    def _onchange_sh_purchase_barcode_mobile(self):
        ...
        if self and self.state in ["cancel","done"] and self.sh_purchase_barcode_mobile:
        ...
        #step 2 increaset product qty by 1 if product not in order line than create new order line.
        elif self and self.sh_purchase_barcode_mobile:
            search_lines = False
            domain = []
            if self.env.user.company_id.sudo().sh_purchase_barcode_mobile_type == "barcode":            
                ...
                search_lines = self.order_line.filtered(lambda ol: ol.product_id.default_code == self.sh_purchase_barcode_mobile)   
                domain = [("default_code","=",self.sh_purchase_barcode_mobile)]
                
            elif self.env.user.company_id.sudo().sh_purchase_barcode_mobile_type == "sh_qr_code":            
                search_lines = self.order_line.filtered(lambda ol: ol.product_id.sh_qr_code == self.sh_purchase_barcode_mobile)   
